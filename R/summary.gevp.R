...
function(object, ...){
   ...
   med=0
   for (i in 1:I)
    {med=med-n*log(sigma[i])-sum((1+xi[i]*(x-mu[i])/sigma[i])^(-1/xi[i]))-(1+1/xi[i])*sum(log((1+xi[i]*(x-mu[i])/sigma[i])))
     }
     med=-2*med/I
     mediaDIC=-n*log(mean(sigma))-sum((1+mean(xi)*(x-mean(mu))/mean(sigma))^(-1/mean(xi)))-(1+1/mean(xi))*sum(log((1+mean(xi)*(x-mean(mu))/mean(sigma))))
     mediaDIC=-2*mediaDIC
     BIC=med+4*log(n)
     pD=med-mediaDIC
     DIC=med+pD
